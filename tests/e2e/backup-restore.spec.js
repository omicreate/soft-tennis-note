...
test.describe("match data backup restore", () => {
  ...
  test("restores match data through the file picker", async ({ page }) => {
    await page.goto("/index.html?v=185");

    const backup = {
      app: "soft-tennis-note",
      schemaVersion: 1,
      appVersion: "v153",
      exportedAt: "2026-05-30T04:30:00.000Z",
      state: {
        matchType: "doubles",
      ...
      archivedMatches: [
        {
          ...
          savedAt: "2026-05-30T04:31:00.000Z",
          title: "読込済み保存試合",
          pointCount: 1,
          finished: false,
          state: {
            ...
            matchInfo: { date: "2026-05-30", tournament: "保存テスト大会" },
            games: { A: 0, B: 0 },
            gamePoints: { A: 0, B: 0 },
            points: []
          }

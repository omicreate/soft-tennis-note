...
assert.match(html, /1 サービス側[\s\S]*2 サービス結果[\s\S]*3 誰のプレー[\s\S]*自動判定（必要なら修正）[\s\S]*4 ポイントの決まり方[\s\S]*得点になったプレー[\s\S]*ストローク[\s\S]*打ち合い[\s\S]*ミスになったプレー[\s\S]*5 得点した側を押して保存/, "かんたん入力は試合中に押す順で表示する");
...
assert.match(html, /class="record-top-grid"[\s\S]*1 サービス側[\s\S]*2 サービス結果/, "記録画面の上段入力はサービス側と結果を並べて表示する");
assert.match(html, /class="control-group player-record-group"[\s\S]*3 誰のプレー[\s\S]*id="playerSavePreview"/, "かんたん記録で誰として保存されるか確認できる");

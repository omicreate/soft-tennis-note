...
assert.match(appJs, /const drawScoreBoard = \(y\) => [\s\S]*return y \+ 286;/, "試合結果枠と次の見出しに十分な余白を残す");
...
assert.match(html, /選手別 成長メモ[\s\S]*選手ごとの \+ \/ - と貢献差、記録したプレー種別/, "選手別分析で貢献差とプレー種別を見る意図を説明する");
...
assert.match(css, /\.pm-total-badge[\s\S]*background:\s*var\(--ink\)[\s\S]*color:\s*#fff[\s\S]*font-weight:\s*1000[\s\S]*white-space:\s*nowrap/, "個人別の貢献差を白文字バッジで表示する");
assert.match(css, /\.sr-card-score[\s\S]*\.sr-card-score \.plus[\s\S]*\.sr-card-score \.minus[\s\S]*\.sr-total-badge[\s\S]*background:\s*var\(--ink\)[\s\S]*color:\s*#fff/, "選手別S/Rは通常の+/-と貢献差バッジを分けて表示する");
assert.match(appJs, /function formatContributionDiff\(diff\)[\s\S]*return `貢献差 \$\{formatPointDiff\(diff\)\}`/, "貢献差の表示名を共通関数で管理する");
assert.match(appJs, /drawText\(`\+\$\{item\.plus\}`[\s\S]*drawText\(`-\$\{item\.minus\}`[\s\S]*drawWhiteBadge\(formatContributionDiff\(item\.diff\)/, "サマリー画像のS/Rは通常の+/-と貢献差バッジを分けて描画する");
assert.match(appJs, /const totalText = plusMinusValueParts\(value\)\.total[\s\S]*drawWhiteBadge\(totalText,[\s\S]*drawWhiteBadge\(parts\.total,/, "サマリー画像の個人別+/-は貢献差だけ白抜きバッジで描画する");

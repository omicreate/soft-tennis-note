...
assert.match(html, /app-config\.js\?v=141[\s\S]*app-analysis\.js\?v=141[\s\S]*app-storage\.js\?v=141[\s\S]*app\.js\?v=141/, "設定、分析、保存、本体の順に読み込む");
assert.match(html, /styles\.css\?v=141/, "styles.cssのキャッシュ更新バージョンが最新");
assert.match(fs.readFileSync("sw.js", "utf8"), /soft-tennis-logger-v141/, "Service Workerのキャッシュ名が最新");
